...
варианты
1. от центрального периферийному. Если отправитель равен 0, он передает данные периферийному
2. от периферийного центральному. Если отправитель не 0, передает данные 0
3. от периферийному периферийному. Передаем данные от отправителя центру (0), а он передает получателю
...
Мы видим такие результаты, потому что в последовательной версии данные просто копируются в памяти без межпроцессных взаимодействий, что гораздо быстрее(мы ведь просто вектор передаем).
